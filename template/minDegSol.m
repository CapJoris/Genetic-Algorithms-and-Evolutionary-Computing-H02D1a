function [totDist,path] = minDegSol()
...
path = int64([minI]);
pathC = int64(1:NVAR);
pathC = pathC(pathC~=minI);%Complement (i.e. elements not in path)
totDist = 0;
...
for el = 1:(NVAR-1)
    [distboi,newNode] = minDeg(path(el),path,Dist);
    path = [path newNode];
    totDist = totDist + distboi;
    %pathC = pathC(pathC~=newNode);
